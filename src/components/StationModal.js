...
export const StationModal = ({ data, tankTypes, close, toast, settings, refresh }) => { 
    ...
    // Deep Copy 'data' so editing form doesn't change 'stations' context/history 
    const [form, setForm] = useState(() => { 
        if (data.station_id) { 
            return JSON.parse(JSON.stringify(data)); 
        } 
        return { name:'', location:'', theme:'bpcl', manager_user:'', manager_pass:'', tanks:[], subscription_status: 'active' }; 
    }); 
    ...
    const save = async () => { 
        ...
        if (!isNew) { 
            ...
            const oldStatus = oldSt.subscription_status || 'active';
            if(oldStatus !== stationData.subscription_status) {
                changes.push(`- Status: ${oldStatus} ➝ ${stationData.subscription_status}`);
            }
            ...
            for (let i = 0; i < maxLen; i++) { 
                ...
                if (!oldT && newT) { changes.push(`+ Added Tank: "${newT.name}" (\`${newT.type}\`)`); continue; } 
                if (oldT && !newT) { changes.push(`- Removed Tank: "${oldT.name}" (\`${oldT.type}\`)`); continue; } 
                if (oldT && newT) { 
                    if (oldT.name !== newT.name) { changes.push(`~ Tank #${i+1} Renamed: "${oldT.name}" ➝ "${newT.name}"`); } 
    ...
    const updateTank = (index, field, value) => { 
        const updatedTanks = form.tanks.map((tank, i) => { 
            if (i === index) { return { ...tank, [field]: value }; } 
            return tank; 
        }); 
        ...
        <div className="modal-overlay"> 
            <div className="modal-box"> 
                ...
                <div style={{display:'grid', gridTemplateColumns:'2fr 1fr 1fr', gap:20}}> 
                    ...
                    <div>
                        <label>Status</label>
                        <select className="app-input" value={form.subscription_status || 'active'} onChange={e=>setForm({...form, subscription_status:e.target.value})}>
                ...
                {/* SCROLLABLE TANK LIST CONTAINER */}
                <div style={{
                    maxHeight: '160px', /* Limits height to roughly 3 tanks */
                    overflowY: 'auto',  /* Enables vertical scrolling */
                    marginBottom: '10px',
                    paddingRight: '5px' /* Adds tiny padding so scrollbar doesn't overlap borders */
                }}> 
                    {(form.tanks || []).map((t, i) => ( 
                        <div key={i} style={{display:'flex', gap:10, marginBottom:10}}> 
                            <input className="app-input" value={t.name} onChange={e => updateTank(i, 'name', e.target.value)} placeholder="Tank Name" style={{marginBottom:0}} /> 
                            <select className="app-input" value={t.type} onChange={e => updateTank(i, 'type', e.target.value)} style={{marginBottom:0}} > 
                ...
                {/* PINNED "ADD TANK" BUTTON (Outside the scroll container) */}
                <button className="btn btn-secondary" style={{width:'100%', borderStyle:'dashed', borderColor:'var(--primary)', color:'var(--primary)', marginBottom: 20}} onClick={addTank}>
                    + Add Tank
                </button> 

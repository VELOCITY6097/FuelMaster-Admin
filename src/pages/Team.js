...
import { Edit2, Trash2, Shield, UserPlus, AlertTriangle, MessageSquare, Fingerprint } from 'lucide-react'; 
import { DataContext } from '../context/DataContext'; 
import { ToastContext, UIContext } from '../context/UIContext'; 
import { AuthContext } from '../context/AuthContext'; 
import { supabase } from '../supabase'; 
...
export const Team = () => { 
    ...
    // --- OPEN ADD/EDIT MODAL --- 
    const openModal = (data = null) => { 
        setForm(data ? { ...data } : { id: null, name: '', phone: '', pin: '', role: 'Staff', discord: '' }); 
        setModal({ open: true, data }); 
    }; 
    ...
    const saveMember = async () => { 
        ...
        if (isNew) { 
            const { error: insertErr } = await supabase.from('admins').insert([{ name: form.name, phone: form.phone, pin: form.pin, role: form.role, discord: form.discord }]); 
            error = insertErr; 
            logEvent = 'TEAM_ADD'; 
            logData = { name: form.name, role: form.role, targetDiscord: form.discord, user: user.name, discordId: user.discord }; 
        } else { 
            const oldData = admins.find(a => a.id === form.id) || {}; 
            let changes = []; 
            if (oldData.name !== form.name) changes.push(`- Name: ${oldData.name} ➝ ${form.name}`); 
            ...
            const { error: updateErr } = await supabase.from('admins').update({ name: form.name, phone: form.phone, pin: form.pin, role: form.role, discord: form.discord }).eq('id', form.id); 
            error = updateErr; 
            logEvent = 'TEAM_UPDATED'; 
            logData = { name: form.name, changes: changes.length > 0 ? changes.join('\n') : "Profile details updated (No visible changes detected)", user: user.name, discordId: user.discord }; 
        } 
    ...
    const confirmRemove = async () => { 
        ...
        if (error) { 
            ...
            toast("Removed Successfully", "success"); 
            sendLog(settings.webhook_url, 'TEAM_REMOVE', { name: admin.name, targetDiscord: admin.discord, reason: reason, user: user.name, discordId: user.discord }); 
            refresh(); 
            setDeleteModal({ open: false, admin: null, reason: '' }); 
        } 
    }; 
    ...
    return ( 
        <div style={{ animation: 'fadeIn 0.5s ease-in-out' }}> 
            {/* RESPONSIVE HEADER */} 
            <div style={{display:'flex', flexWrap: 'wrap', justifyContent:'space-between', alignItems:'center', gap: 15, marginBottom: 25}}> 
                <div>
                    <h2 style={{margin: '0 0 5px 0', display:'flex', alignItems:'center', gap:10, fontSize: '1.8rem', letterSpacing: '-0.5px'}}> 
                        <Shield size={28} color="var(--primary)"/> Security Team 
                    </h2> 
                    <p style={{ margin: 0, color: 'var(--text-muted)', fontSize: '0.95rem' }}>Manage system access and roles.</p>
                </div>
                {user.role === 'Owner' && ( 
                    <button className="btn btn-primary" style={{padding: '10px 20px'}} onClick={()=>openModal(null)}> 
                        <UserPlus size={18} style={{marginRight:8}}/> Add Member 
                    </button> 
                )} 
            </div> 

            {/* RESPONSIVE CARD GRID (Replaced the Table) */} 
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(100%, 320px), 1fr))', gap: 20 }}>
                {admins.map(a => {
                    const isOwnerBadge = a.role === 'Owner';
                    const isModBadge = a.role === 'Moderator';
                    
                    return (
                    <div key={a.id} className="card" style={{ display: 'flex', flexDirection: 'column', gap: 15 }}>
                        {/* Header: Name + Role */}
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                            <div>
                                <h3 style={{ margin: '0 0 4px 0', fontSize: '1.15rem' }}>{a.name}</h3>
                                <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: 5 }}>
                                    <Fingerprint size={14}/> ID: <span style={{fontFamily: 'monospace', color: 'var(--text-main)'}}>{a.phone}</span>
                                </div>
                            </div>
                            <span style={{ 
                                background: isOwnerBadge ? '#fef3c7' : (isModBadge ? '#e0e7ff' : '#f1f5f9'), 
                                color: isOwnerBadge ? '#d97706' : (isModBadge ? '#4f46e5' : '#64748b'), 
                                padding: '4px 10px', borderRadius: 20, fontSize: '0.75rem', fontWeight: 700, 
                                border: `1px solid ${isOwnerBadge ? '#fcd34d' : (isModBadge ? '#c7d2fe' : '#cbd5e1')}` 
                            }}> 
                                {a.role.toUpperCase()} 
                            </span> 
                        </div>
                        ...
                        {/* Discord Linked Info */}
                        <div style={{ background: 'var(--bg-body)', padding: '10px 14px', borderRadius: 8, fontSize: '0.85rem', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: 8, border: '1px solid var(--border)' }}>
                            <MessageSquare size={16} color="var(--text-muted)"/>
                            Discord: 
                            <span style={{ fontFamily: 'monospace', color: a.discord ? 'var(--text-main)' : 'var(--text-muted)', fontWeight: a.discord ? 600 : 400 }}>
                                {a.discord || 'Not Linked'}
                            </span>
                        </div>
                        ...
                        {user.role === 'Owner' && ( 
                            <div style={{ display: 'flex', gap: 10, marginTop: 'auto', paddingTop: 10 }}> 
                                <button className="btn btn-secondary" style={{ flex: 1, display: 'flex', justifyContent: 'center', gap: 8, padding: 10 }} onClick={()=>openModal(a)}>
                                    <Edit2 size={16}/> Edit Profile
                                </button> 
                                {a.phone !== BOSS_PHONE && ( 
                                    <button className="btn btn-danger" style={{ padding: '10px 15px' }} onClick={()=>initiateRemove(a)}>
                                        <Trash2 size={16}/>
                                    </button> 
                                )} 
                            </div> 
            ...
            {/* ADD/EDIT MODAL (Responsive Form) */} 
            {modal.open && ( 
                <div className="modal-overlay"> 
                    <div className="modal-box" style={{ width: '90%', maxWidth: '450px', padding: 25 }}> 
                        <h3 style={{marginTop: 0}}>{modal.data ? 'Edit' : 'Add'} Team Member</h3> 
                        <div style={{marginTop:20, display:'flex', flexDirection:'column', gap:15}}> 
                            <div> 
                                <label>Full Name</label> 
                                <input className="app-input" value={form.name} onChange={e=>setForm({...form,name:e.target.value})} placeholder="Staff Name" style={{marginBottom:0, width: '100%', boxSizing: 'border-box'}}/> 
                            </div> 
                            <div> 
                                <label>Login ID (Phone/Username)</label> 
                                <input className="app-input" value={form.phone} onChange={e=>setForm({...form,phone:e.target.value})} placeholder="e.g. Unique User ID" disabled={form.phone===BOSS_PHONE} style={{marginBottom:0, width: '100%', boxSizing: 'border-box'}}/> 
                            </div> 
                            <div> 
                                <label>Secure Password / PIN</label> 
                                <input className="app-input" value={form.pin} onChange={e=>setForm({...form,pin:e.target.value})} placeholder="Alphanumeric (Min 4 chars)" maxLength={16} style={{marginBottom:0, width: '100%', boxSizing: 'border-box'}} /> 
                                <div style={{fontSize:'0.7rem', color:'var(--text-muted)', marginTop:4}}>Max 16 characters</div> 
                            </div> 
                            <div> 
                                <label>Discord ID (Optional)</label> 
                                <input className="app-input" value={form.discord} onChange={e=>setForm({...form,discord:e.target.value})} placeholder="For system log tagging" style={{marginBottom:0, width: '100%', boxSizing: 'border-box'}}/> 
                            </div> 
                            <div> 
                                <label>Access Role</label> 
                                <select className="app-input" value={form.role} onChange={e=>setForm({...form,role:e.target.value})} disabled={form.phone===BOSS_PHONE} style={{marginBottom:0, width: '100%', boxSizing: 'border-box'}}> 
                                    <option value="Staff">Staff (Stations Only)</option> 
                                    <option value="Moderator">Moderator (Full Access)</option> 
                                </select> 
                            </div> 
                        ...
                        <div style={{display:'flex', flexWrap: 'wrap', justifyContent:'flex-end', gap:10, marginTop:25}}> 
                            <button className="btn btn-secondary" style={{ flex: '1 1 auto' }} onClick={()=>setModal({open:false, data:null})}>Cancel</button> 
                            <button className="btn btn-primary" style={{ flex: '1 1 auto' }} onClick={saveMember}>Save User Profile</button> 
                        </div> 
                    </div> 
                </div> 
            )} 
            ...
            {deleteModal.open && ( 
                <div className="modal-overlay" style={{zIndex:9999}}> 
                    <div className="modal-box" style={{ width: '90%', maxWidth: '400px', padding: 25 }}> 
                        <div style={{display:'flex', alignItems:'center', gap:10, color:'#ef4444', marginBottom:15}}> 
                            <AlertTriangle size={24}/> <h3 style={{margin:0}}>Revoke Access?</h3> 
                        </div> 
                        <p style={{color:'var(--text-muted)', fontSize:'0.9rem', lineHeight:1.5}}> 
                            Are you sure you want to remove <strong>{deleteModal.admin?.name}</strong>? This action will immediately block their login capabilities. 
                        </p> 
                        <label style={{marginTop:20, display:'block', fontSize:'0.85rem', fontWeight:600}}>Reason for Removal (Required)</label> 
                        <input className="app-input" placeholder="e.g. Resigned, Policy Violation..." value={deleteModal.reason} onChange={e=>setDeleteModal({...deleteModal, reason:e.target.value})} autoFocus style={{width: '100%', boxSizing: 'border-box'}}/> 
                        
                        <div style={{display:'flex', flexWrap: 'wrap', justifyContent:'flex-end', gap:10, marginTop:25}}> 
                            <button className="btn btn-secondary" style={{ flex: '1 1 auto' }} onClick={()=>setDeleteModal({open:false, admin:null, reason:''})}>Cancel</button> 
                            <button className="btn btn-danger" style={{ flex: '1 1 auto' }} onClick={confirmRemove}>Confirm Removal</button> 
                        </div> 
                    </div> 
                </div> 
            )} 
